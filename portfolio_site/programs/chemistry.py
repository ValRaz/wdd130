...
def make_periodic_table():
   periodic_table_dict = {"Ac": ["Actinium", 227, 89],
...
def get_formula_name(chem_formula, known_molecules_dict):
    ...
    for formula, name in known_molecules_dict.items():
        if chem_formula in known_molecules_dict:
            compound_name = known_molecules_dict[chem_formula]
...
def main():
    """Get chemical formula for a molecule and mass of a sample in grams from a user, call the make_periodic_table function and store the returned list as a variable"""
    chem_formula = input('Please provide the chemical formula for your molecule:\n')
    chem_mass = float(input('What is the mass of your sample in grams?\n'))
    periodic_table = make_periodic_table()
    symbol_quantity_list = parse_formula(chem_formula,periodic_table)
    known_molecules = make_known_molecules_dict()

    formula_name = get_formula_name(chem_formula, known_molecules)

    molar_mass = compute_molar_mass(symbol_quantity_list, periodic_table)

    moles = chem_mass / molar_mass

    total_protons = sum_protons(symbol_quantity_list, periodic_table)

    print(f'The total molar mass for {formula_name} is {molar_mass} grams/mole, the number of moles is {moles}. The total number of protons is {total_protons}.')
