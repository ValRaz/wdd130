...
def main():
    ...
    #readable count of marriages per person.
...
def print_death_age(people_dict):
    ...
    Return: nothing
    """
    for person_key, person_value_list in people_dict.items():

        name= person_value_list[NAME_INDEX]
        birth_year = person_value_list[BIRTH_YEAR_INDEX]
        death_year = person_value_list[DEATH_YEAR_INDEX]

        age_died = death_year - birth_year

        print(name, birth_year, death_year, age_died)
...
def count_genders(people_dict):
    ...
    for person_key, person_value_list in people_dict.items():
        gender = person_value_list[GENDER_INDEX]
        if gender == "M":
            count_of_males += 1
    
    total_people_count = len(people_dict)
    
    count_of_females = total_people_count - count_of_males

    print(f"""Gender count: 
          Males- {count_of_males}.
          Females- {count_of_females}.""")
...
def print_marriages(marriages_dict, people_dict):
    ...
    Return: nothing
    """
    for marriage_key, marriage_value_list in marriages_dict.items():

        husband_key = marriage_value_list[HUSBAND_KEY_INDEX]
        wife_key = marriage_value_list[WIFE_KEY_INDEX]
        year_married = marriage_value_list[WEDDING_YEAR_INDEX]

        husband_list = people_dict[husband_key]
        husband_name = husband_list[NAME_INDEX]
        husband_year_born = husband_list[BIRTH_YEAR_INDEX]
        wife_list = people_dict[wife_key]
        wife_name = wife_list[NAME_INDEX]
        wife_year_born = wife_list[BIRTH_YEAR_INDEX]

        husband_age_married = year_married - husband_year_born
        wife_age_married = year_married - wife_year_born

        print(f"""Marriages: 
              Husband- {husband_name}, {husband_age_married}, {year_married}
              Wife- {wife_name}, {wife_age_married}""")
...
def count_marriages(marriages_dict, people_dict):
    ...
    for marriage_key, marriage_list in marriages_dict.items():
        ...
        husband_list = people_dict[husband_key]
        husband_list[MARRIAGE_COUNT_INDEX] +=1
        wife_list = people_dict[wife_key]
        wife_list[MARRIAGE_COUNT_INDEX] +=1

    for person_key, person_list in people_dict.items():
        name = person_list[NAME_INDEX]
        count_of_marriages = person_list[MARRIAGE_COUNT_INDEX]

        print(f"""Times Married:
              {name}- {count_of_marriages}.""")
